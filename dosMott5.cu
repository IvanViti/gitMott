...
*/
//      if ((distance > 0) && (distance < 20)) cout<<distance<<endl;
}
...
//second part of the heart of this code. Here the probabilities are summed and a number is picked from 0 to that number. The code then sums through the probabilities untill it reaches that number. In this way, probabilities which are higher will have a larger chance of getting picked. 
 __global__ void particleMove(REAL* particles,REAL* probabilities,int x,int y,int N,double randomNum) {
        int newx,newy,lastx,lasty,found,i,j;
        double summation,randNum;
//	int idx=(blockIdx.y*gridDim.x+blockIdx.x)*blockDim.x+threadIdx.x;
        ...
        for (j = 0; j < N; j++) {
                for (i = 0 ; i < N; i++) {
                        summation = summation + probabilities[i + N*j];
                }
        }

	        randNum = summation*randomNum; //probably need better names
       // cout<<"summation is "<<summation<<endl;
	summation = 0; //reuse
      for (j = 0; j < N; j++) {
                for (i = 0 ; i < N ; i++) {
//                      cout<<"summation is "<< summation<<". random number is "<<randNum<<endl;
                        if ((summation > randNum) && (found == 0)) {
                                found = 1;
                                if (j > 0) {
                                        lastx = i - 1;
                                        lasty = j;
                        ...
                        summation = summation + probabilities[i + j*N] ;
...
//	newx = lastx;
//	newy = lasty;
//        showJump( N, x,y, newx,newy,hereP);
...
void findJump(REAL* hereP,REAL* hereProb,REAL* herePot,REAL *particles,REAL *probabilities,REAL *potentials,REAL *substrate,int N,double xi,int threads,int blocks,double eV,double Ec,double L,double T,REAL *boxR, double alphaOne, double alphaTwo) {
	int x,y;	
	double randomNum;
	 x = floor(drand48()*N);
         y = floor(drand48()*N);
...
//	cudaMemcpy(herePot,potentials,N*N*sizeof(REAL),cudaMemcpyDeviceToHost);
	randomNum = drand48();
	particleMove<<<blocks,threads>>>(particles,probabilities,x,y,N,randomNum);
...
//	relax = 1;
	...
	REAL *particles,*probabilities,*potentials,*substrate,*hereP,*hereProb,*herePot,*hereS,*boxR,*hereBoxR,*hereXDiff,*hereYDiff,*dosMatrix,*reducedSum,*g_itemp,*g_otemp,*g_temp,*hereDos;
	xi = L/sqrt(sqrt(2));
	xVar = L;
	yVar = L;
...
//      dosMatrix = dosFind(hereP, hereS,herePot,dosMatrix, particles,potentials,boxR, N, L, threads, blocks);  
  //    showMove(dosMatrix,N);  
	...
	for(t = 0; t < tSteps ; t++) {
		countThese = 1;
		findJump(hereP,hereProb,herePot,particles,probabilities,potentials,substrate, N, xi, threads, blocks,eV,Ec,L,T,boxR,alphaOne,alphaTwo);
	}
